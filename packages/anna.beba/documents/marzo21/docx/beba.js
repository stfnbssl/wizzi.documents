/*
    ...
    package: wizzi-js@
    primary source IttfDocument: C:\My\wizzi\stfnbssl\wizzi.documents\packages\anna.beba\.wizzi-override\documents\marzo21\docx\beba.js.ittf
    utc time: Wed, 24 May 2023 15:52:36 GMT
*/
'use strict';
...
function executeMain() {
    executeGenerateModules([
        ...
        if (err) {
            console.log('beba.executeGenerateModules.err', err, __filename);
            console.log('beba.executeGenerateModules.err.toString()', err.toString(), __filename);
            if (err.inner) {
                console.log('beba.executeGenerateModules.err.inner.toString()', err.inner.toString(), __filename);
            }
        }
        else {
        }
    ...
    function executeGenerateModules(modules, callback) {
        async.mapSeries(modules, (moduleFileName, callback) => {
            ...
            var context = file.readJSON(path.join(__dirname, 'data', 'extracted', moduleFileName + '.detail.json'));
            context.items.sort(sortItems);
            console.log('Beba.started ', moduleFileName, __filename);
            transformContext(context, (err, context) => {
            
                if (err) {
                    return callback(err);
                }
                console.log('Beba.transformed context', moduleFileName, __filename);
                var outputPath = path.join(__dirname, moduleFileName + '_marzo_21.docx.js');
                var outputPathJson = path.join(__dirname, 'debug', moduleFileName + '.docx.json');
                var outputPathDebugTxt = path.join(__dirname, 'dist', 'beba', 'test', moduleFileName + '.docx.debug.txt');
                var outputPathBuildUpIttf = path.join(__dirname, 'debug', moduleFileName + '.docx.buildup.ittf');
                ...
                console.log('Beba.generating docx script', moduleFileName, __filename);
                loadModelAndGenerateArtifactFromText(ittfText, {
                    ...
                    console.log('Generated docx script', outputPath, __filename);
    ...
    function buildTestoDocxIttf(item, indent, sb, prev, ctx) {
        ...
        // done
            ...
            if (item.name != 'html') {
                console.log('+++++++++++++++++++++. Unknown', item.name, __filename);
    ...
    function buildArticleImages(item, indent, sb) {
        if (item.images && item.images.length > 0) {
            ...
            for (i=0; i<i_len; i++) {
                image = item.images[i];
                console.log('buildArticleImages, image', image, __filename);
    ...
    function preprocessTesto(item, ctx) {
        ...
        if (item.name == 'span') {
            console.log('preprocessTesto.span', item.name, item.value, item.children.length, __filename);
            var i, i_items=item.children, i_len=item.children.length, child;
            for (i=0; i<i_len; i++) {
                child = item.children[i];
                console.log('preprocessTesto.span child', child.name, child.value, child.children.length, __filename);
            }
            for (var k in item.attribs) {
                console.log('preprocessTesto.span attribute', k, item.attribs[k], __filename);
            }
        }
    ...
    function transformContext(beba, callback) {
        async.mapSeries(beba.items, (item, callback) => {
            ...
            getWizziTreeFromText(itemTesto, 'html', (err, mtree) => {
    ...
    function dumpIttfJson(node, indent) {
        if (node.tag) {
            console.log(new Array(1+indent||1).join(' ') + (node.tag), '|' + (node.name || '') +'|', __filename);
        }
        else {
            console.log(new Array(1+indent||1).join(' ') + (node.name), '|' + (node.value || '') +'|', __filename);
        }
...
function getWizziIttf(documentUri, wizzifierName, plugins, callback) {
    ...
    createWizziFactory({}, plugins, (err, wf) => {
        ...
        wf.getWizziIttf(documentUri, wizzifierName, callback)
    }
    )
}
function getWizziIttfFromText(documentText, wizzifierName, plugins, callback) {
    ...
    createWizziFactory({}, plugins, (err, wf) => {
        ...
        wf.getWizziIttfFromText(documentText, wizzifierName, callback)
    }
    )
}
function getWizziTree(documentUri, wizzifierName, plugins, callback) {
    ...
    createWizziFactory({}, plugins, (err, wf) => {
        ...
        wf.getWizziTree(documentUri, wizzifierName, callback)
    }
    )
}
function getWizziTreeFromText(documentText, wizzifierName, plugins, callback) {
    ...
    createWizziFactory({}, plugins, (err, wf) => {
        ...
        wf.getWizziTreeFromText(documentText, wizzifierName, callback)
    }
    )
}
function getCodeAst(documentUri, wizzifierName, plugins, callback) {
    ...
    createWizziFactory({}, plugins, (err, wf) => {
        ...
        wf.getCodeAst(documentUri, wizzifierName, callback)
    }
    )
}
function getCodeAstFromText(documentText, wizzifierName, plugins, callback) {
    ...
    createWizziFactory({}, plugins, (err, wf) => {
        ...
        wf.getCodeAstFromText(documentText, wizzifierName, callback)
    }
    )
}
